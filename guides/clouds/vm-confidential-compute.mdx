...
description: "Run secure AI workloads with Intel TDX encryption on io.cloud. Protect data in use on H100, H200, and B200 GPUs at up to 90% lower cost."
---
...
[io.net](http://io.net) delivers the same enterprise-grade protection as _Azure_ and _Google Cloud_ Confidential VMs, at a fraction of the cost, with bare-metal GPU performance and no centralized dependency.
...
_Big Tech_ encrypts data at rest and in transit, but not during processing, when your most sensitive data is exposed.
...
Confidential Compute is currently rolling out for select users, with self-serve deployment available after **November 24**.

To begin using **Confidential Compute** on [io.net](http://io.net):

1. **Request Access:** Submit a provisioning request through[ ](https://io.cloud)[io.cloud](http://io.cloud). Choose your preferred GPU type (H100, H200, or B200) and provide basic project details.

   <Note>
     Approvals are typically issued within 24 hours.
   </Note>
2. **Deploy a Confidential VM:** Once approved, deploy your Confidential VM using the web console, CLI, or API.
...
4. **Integrate Seamlessly:** Run workloads on **Ubuntu 25.04** with **built-in Intel TDX support**. Existing MLOps pipelines run without modification. You can migrate existing workflows directly to [io.net](http://io.net) without changing your training or deployment code.
