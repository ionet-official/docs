---
title: "Confidential Compute Attestation"
description: "Learn what confidential compute attestation is and how it can be utilized to verify secure workloads on io.net Cloud. Explore Trusted Execution Environments (TEE), GPU verification, and hardware-based security for sensitive compute."
    ...
    **What this proves:**\
    ...
    <Accordion title="Further Reading on CC and PPCIe">
      **How CC and Protected PCIe (PPCIe) Work Together**
      ...
      These settings determine _how_ and _which_ GPUs operate in confidential mode. Each GPU can have CC and PPCIe enabled or disabled independently, but only certain combinations are valid.
      ...
      - **CC = OFF, PPCIe = ON**\
        ...
        <Note>
          _This is the configuration used in our recommended setup._
        </Note>
      - **CC = ON, PPCIe = ON**\
...
<AccordionGroup>
  <Accordion title="How often should verification be performed?" icon="comment-question">
    ...
    **Why regular verification is important:**
    ...
    - Confirms that _Confidential Computing_ features remain enabled after updates.
