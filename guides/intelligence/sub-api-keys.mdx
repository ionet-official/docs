---
title: "Sub-API Keys"
description: "Issue scoped API keys to teams, services, or partners — each with independent model restrictions and credit limits that roll up to your account balance."
sidebarTitle: "Sub-API Keys"
---

Sub-API keys allow an account holder (the _admin_) to issue multiple child API keys, each independently controlled. Each sub-key can be restricted to specific models, given a spending cap per billing cycle, and revoked at any time — without touching the admin key or other sub-keys.
