...
The same field works for every model that supports reasoning — no per-model configuration required. Pass a reasoning object with an `effort value` to control how deeply the model thinks before responding.
...
The reasoning enabling/disable capability is active for the following models currently:
...
<AccordionGroup>
  <Accordion title="Do I need different code for different models?" icon="comment-question">
    No. Simply use `reasoning.effort` everywhere, IO Intelligence handles the rest.
  </Accordion>
